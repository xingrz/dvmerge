...
def merge_log(files, output=None, *, no_cache=False, cache_dir=None, dvrescue_bin=None,
    ...
    otherwise a fresh cache entry is reused when present and the merged DV goes to a temp file that
    is removed once parsed. ``cache_dir`` defaults to ``<dir-of-first-input>/.dvmerge``. ``cleanup()``
    removes any temp artifacts.
    """
...
def analyze(files, *, output=None, fps=DEFAULT_FPS, bridge_s=3.0, min_s=0.5,
    ...
    try:
        ...
        return planmod.build(frames, list(files), fps, bridge_s=bridge_s, min_s=min_s)
    finally:
        cleanup()
